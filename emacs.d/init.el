...
;; Define he following variables to remove the compile-log warnings when defining ido-ubiquitous
...
(defvar my-packages
  '(;; makes handling lisp expressions much, much easier Cheatsheet:
    ...
    ;; allow ido usage in as many contexts as possible. see customizations/navigation.el
    ;; line 23 for a description of ido
    ido-completing-read+

    ;; Enhances M-x to allow easier execution of commands. Provides a filterable list of
    ;; possible commands in the minibuffer http://www.emacswiki.org/emacs/Smex
    smex
...
;; different environment than a shell in a terminal window, because OS X does not run a
;; shell during the login. Obviously this will lead to unexpected results when calling
;; external utilities like make from Emacs.  This library works around this problem by
...
;; Place downloaded elisp files in ~/.emacs.d/site-lisp. You'll then be able to load them.
...
;; Add directories to our load path so that when you `load` things below, Emacs knows
;; where to look for the corresponding files.
...
;; These customizations  make it  easier for  you to navigate  files, switch  buffers, and
;; choose options from the minibuffer.
(load "navigation.el")

;; These customizations change the way emacs looks and disable/enable some user interface
;; elements
(load "ui.el")
